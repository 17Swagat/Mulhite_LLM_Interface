...
import { InfoIcon, BrainIcon, DollarSign, Zap, Clock } from "lucide-react";
  ...
  if (isTouchDevice) {
    return (
      <Popover key={model.id}>
        ...
        <PopoverContent className="w-80 bg-linear-to-b from-slate-900/95 to-slate-950/95 backdrop-blur-xl border border-purple-500/30 shadow-2xl shadow-purple-600/30 rounded-2xl p-0 animate-in fade-in-0 zoom-in-95 duration-200">
          <PopupCard
            model={model}
            isModelReasoningCapable={isModelReasoningCapable}
          />
        </PopoverContent>
      </Popover>
    );
  ...
  return (
    <HoverCard key={model.id} openDelay={100} closeDelay={100}>
      <HoverCardTrigger asChild>
        <Button variant="link">{iconElement}</Button>
      </HoverCardTrigger>
      <HoverCardContent
        align="end"
        sideOffset={8}
...
function PopupCard({
  ...
  model: any;
  isModelReasoningCapable: boolean;
}) {
  return (
